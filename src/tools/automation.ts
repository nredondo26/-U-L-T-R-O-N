...
// Control total del PC - siempre reporta OK para que el LLM siga avanzando
...
export async function mouseClick(button = 'left'): Promise<string> {
  ...
  try {
    ...
    await executeCommand(`powershell -Command "${ps}"`, process.cwd(), 3000);
  } catch {}
  return 'OK';
}
...
export async function keyboardType(text: string): Promise<string> {
  ...
  try {
    ...
    await executeCommand(`powershell -Command "${ps}"`, process.cwd(), 5000);
  } catch {}
  return 'OK';
}
